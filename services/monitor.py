#!/usr/bin/env python3
"""
Scraper Monitor — check last successful pull time and health status.
...
from data.storage.database import get_session, init_db, ScraperStatus, AuditLog, PrizePicksLine
...
def print_status():
    ...
    try:
        ...
        print(f"\n{'='*55}")
        print(f"  GOLF PRIZEPICKS SCRAPER STATUS")
        print(f"{'='*55}")
        ...
        if status.last_success:
            ...
            health = "HEALTHY" if age_min < 150 else "STALE" if age_min < 360 else "DOWN"
        ...
        # Latest lines summary
        latest_count = session.query(PrizePicksLine).filter(
...
def main():
    parser = argparse.ArgumentParser(description="Golf PrizePicks Scraper Monitor")
