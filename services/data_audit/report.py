...
class DataQualityReport:
    ...
    def generate(self) -> str:
        ...
        log.info(
            ...
            composite_score, len(critical_findings),
        )
        return report
    ...
    def generate_dict(self) -> Dict[str, Any]:
        ...
        return {
            "composite_score": composite_score,
            "timestamp": {"score": ts_score, **ts_result},
            "odds": {"score": odds_score, **odds_result},
            "closing_line": {"score": closing_score, **closing_result},
            "completeness": {"score": completeness_score, **completeness_result},
            "critical_findings": critical_findings,
    ...
    def _extract_critical_findings(self, ts_result, odds_result, closing_result, completeness_result) -> List[str]:
    ...
    def _format_report(self, composite_score, ts_score, odds_score, closing_score,
                        completeness_score, ts_result, odds_result, closing_result,
                        completeness_result, critical_findings) -> str:
...
Line history depth: {completeness_result.get('line_history_adequate_pct', 0):.1f}% adequate"""
    ...
    def _store_report(self, composite_score, ts_result, odds_result,
                       closing_result, completeness_result, report_text) -> None:
        """Store the audit report in the database."""
        session = self._session()
        try:
            details = {
                "timestamp_audit": {k: v for k, v in ts_result.items() if k != "issues"},
                "odds_audit": {k: v for k, v in odds_result.items()
                               if k not in ("issues", "stale_lines_detail", "phantom_lines_detail")},
                "closing_line_audit": {k: v for k, v in closing_result.items() if k != "issues"},
                "completeness_audit": {k: v for k, v in completeness_result.items()
                                        if k not in ("issues", "systematic_gaps",
                                                      "book_coverage", "market_coverage")},
            }
