...
# PGA Tour runs year-round (January through August/September)
# No real off-season constraint like NBA
GOLF_EARLIEST_YEAR = 2020

# Expected polling interval for odds ingestion (15 minutes for golf)
EXPECTED_POLL_INTERVAL_SEC = 900
# Maximum acceptable gap before flagging as missed poll
MAX_POLL_GAP_SEC = 2700  # 45 minutes (3x expected interval)
...
class TimestampAuditor:
    ...
    def _check_impossible_timestamps(self) -> Dict[str, Any]:
        ...
        try:
            ...
            ancient_cutoff = datetime(GOLF_EARLIEST_YEAR, 1, 1, tzinfo=timezone.utc)
            ancient_count = (
                session.query(sa_func.count(CLVLineMovement.id))
                .filter(
                    CLVLineMovement.sport == self.sport,
            ...
            if ancient_count > 0:
                issues.append(
                    ...
                    f"have timestamps before {GOLF_EARLIEST_YEAR} — likely data errors"
                )

            total = future_count + future_bets + ancient_count

            return {
                "count": total,
                "future_count": future_count + future_bets,
                "pre_season_count": ancient_count,
                "issues": issues,
            }
        finally:
            session.close()
    ...
    def _check_timezone_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check bet timestamps vs snapshot timestamps
            snapshots = (
    ...
    def _check_closing_line_timing(self) -> Dict[str, Any]:
        ...
        For golf: closing lines should be captured at first tee time.
        """
        session = self._session()
        try:
            issues = []
            ...
            for cl in closing_lines:
                if cl.event_start_time and cl.captured_at:
                    ...
                    if delta > 300:  # More than 5 minutes from event start
                        late_count += 1
                    if delta > 1800:  # More than 30 minutes
                        very_early_count += 1
            ...
            if late_count > 0:
                issues.append(
                    ...
                    f"captured more than 5 minutes from first tee time"
                )

            if very_early_count > 0:
                issues.append(
                    f"VERY_EARLY_CLOSINGS: {very_early_count} closing lines captured "
                    f"30+ minutes before first tee — likely stale pre-close data"
                )
