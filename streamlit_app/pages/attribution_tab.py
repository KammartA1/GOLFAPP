...
Displays:
  ...
  - Market inefficiency segment table (golf-specific: tournament, wave, market)
  - CLV diagnostic
  - Final verdict
        ...
        with get_session() as session:
            ...
            for r in rows:
                bets.append({
                    ...
                    "player": getattr(r, "player", "") or "",
                    "tournament": getattr(r, "tournament", "unknown") or "unknown",
                    "market_type": getattr(r, "market", None) or getattr(r, "bet_type", "unknown") or "unknown",
                    "direction": getattr(r, "direction", "over") or "over",
                    "wave": getattr(r, "wave", None),
                    "bet_line": float(getattr(r, "line", 0) or 0),
                    "line": float(getattr(r, "line", 0) or 0),
                    "closing_line": float(r.closing_line) if getattr(r, "closing_line", None) else None,
                    "predicted_prob": float(getattr(r, "model_prob", 0.5) or 0.5),
                    "model_prob": float(getattr(r, "model_prob", 0.5) or 0.5),
                    "market_prob_at_bet": float(getattr(r, "market_prob", 0.5) or 0.5),
                    "market_prob": float(getattr(r, "market_prob", 0.5) or 0.5),
                    "won": r.status == "won",
                    "stake": float(getattr(r, "stake", 0) or 0),
                    "pnl": float(getattr(r, "pnl", 0) or 0),
                    "odds_decimal": float(getattr(r, "odds_decimal", 1.909) or 1.909),
                    "odds_american": int(getattr(r, "odds_american", -110) or -110),
...
def _render_counterfactual_chart(report: AttributionReport) -> None:
    ...
    if _HAS_PLOTLY:
        ...
        values = [
            cf.actual_profit, cf.closing_line_profit,
            cf.timing_edge_dollars, cf.prediction_edge_dollars,
        ]
...
def _render_verdict_card(report: AttributionReport) -> None:
    ...
    st.markdown(f"""
    <div style="background: linear-gradient(135deg, #0c1425, #111b2e);
...
def render() -> None:
    ...
    if not bets:
        ...
        st.info(
            ...
            "3. **Market Inefficiency** — structural mispricing (outright, wave, course)\n"
    ...
    st.markdown(
        "<h4 style='font-family: Inter, sans-serif; color: #3B82F6;'>"
