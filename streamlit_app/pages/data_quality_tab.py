...
========================================
Data Quality Audit dashboard for the Golf Quant Engine.
...
def _get_report():
    ...
    return DataQualityReport(sport="GOLF")
...
def _render_score_gauge(label: str, score: float, col):
    ...
    col.markdown(
        ...
        <div style='font-size:0.55rem;color:{color};'>{_score_label(score)}</div>
        </div>""",
...
def _render_overview(report_service):
    ...
    if not data:
        ...
        if latest:
            ...
            st.markdown(
                ...
                font-family:Chakra Petch,monospace;'>{score:.0f}</div>
                <div style='font-size:0.7rem;color:{color};'>{_score_label(score)}</div>
                </div>""",
                unsafe_allow_html=True,
            )
    ...
    st.markdown(
        ...
        <div style='font-size:0.7rem;color:{color};'>{_score_label(composite)}</div>
        </div>""",
        unsafe_allow_html=True,
    )
...
def _render_timestamp_detail(report_service):
    st.markdown("<div style='font-size:0.8rem;font-weight:600;color:#00FFB2;'>TIMESTAMP AUDIT</div>", unsafe_allow_html=True)
    ...
    if not data:
        ...
        return
    _render_score_gauge("Timestamp Score", data.get("score", 0), st)
    st.markdown("---")
...
def _render_odds_detail(report_service):
    st.markdown("<div style='font-size:0.8rem;font-weight:600;color:#00FFB2;'>ODDS AUDIT</div>", unsafe_allow_html=True)
    data = st.session_state.get("_dq_audit_data", {}).get("odds")
    if not data:
        st.info("Run a full audit from the Overview tab to see odds details.")
        return
    _render_score_gauge("Odds Score", data.get("score", 0), st)
    st.markdown("---")
    c1, c2, c3 = st.columns(3)
    ...
    if stale_details:
        ...
        st.dataframe(pd.DataFrame(stale_details), use_container_width=True, hide_index=True)
    phantom_details = data.get("phantom_lines_detail", [])
    if phantom_details:
        st.markdown("**Phantom Lines:**")
        st.dataframe(pd.DataFrame(phantom_details), use_container_width=True, hide_index=True)
    for issue in data.get("issues", []):
        st.warning(issue)
    if not data.get("issues"):
...
def _render_closing_detail(report_service):
    st.markdown("<div style='font-size:0.8rem;font-weight:600;color:#00FFB2;'>CLOSING LINE AUDIT</div>", unsafe_allow_html=True)
    ...
    if not data:
        ...
        return
    _render_score_gauge("Closing Line Score", data.get("score", 0), st)
    st.markdown("---")
...
def _render_completeness_detail(report_service):
    st.markdown("<div style='font-size:0.8rem;font-weight:600;color:#00FFB2;'>COMPLETENESS AUDIT</div>", unsafe_allow_html=True)
    ...
    if not data:
        ...
        return
    _render_score_gauge("Completeness Score", data.get("score", 0), st)
    st.markdown("---")
    c1, c2, c3, c4 = st.columns(4)
    ...
    fig = go.Figure(go.Bar(
        x=df["Missing %"], y=df["Category"], orientation="h",
        marker_color=["#FF3358" if v > 30 else "#FFB800" if v > 10 else "#00FFB2" for v in df["Missing %"]],
        text=[f"{v:.1f}%" for v in df["Missing %"]], textposition="auto",
    ))
    fig.update_layout(
...
def _render_history(report_service):
    st.markdown("<div style='font-size:0.8rem;font-weight:600;color:#00FFB2;'>AUDIT HISTORY</div>", unsafe_allow_html=True)
    ...
    fig.add_trace(go.Scatter(
        x=df["generated_at"], y=df["integrity_score"],
        mode="lines+markers", name="Quality Score",
        line=dict(color="#00FFB2", width=2), marker=dict(size=6),
    ))
    fig.add_hline(y=80, line_dash="dash", line_color="#FF3358",
                  annotation_text="Critical Threshold", annotation_position="top left",
                  annotation_font_color="#FF3358")
    fig.add_hline(y=90, line_dash="dot", line_color="#FFB800",
                  annotation_text="Good Threshold", annotation_position="top left",
                  annotation_font_color="#FFB800")
    fig.update_layout(
        height=350, margin=dict(l=0, r=0, t=30, b=10),
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        font=dict(family="Fira Code, monospace", size=11, color="#4A607A"),
    ...
    display_df.columns = ["Date", "Score", "Bets", "Missing Open", "Missing Close", "Data Errors"]
...
def _render_raw_report(report_service):
    st.markdown("<div style='font-size:0.8rem;font-weight:600;color:#00FFB2;'>RAW REPORT</div>", unsafe_allow_html=True)
